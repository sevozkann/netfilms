...
async function fetchData(path, { query = "" } = {}) {
  ...
  try {
    const res = await fetch(
      `${BASE_URL}${path}?api_key=c40dcad2d043ed352fed39a6298561ab&${query}`
    );
    return res.json();
  } catch (error) {
...
export async function fetchMovieReviews(movieId) {
  try {
    const res = await fetchData(`/movie/${movieId}/reviews`);
    return res;
  } catch (error) {
    throw new Error("Error happened while fetching top rated movies", error);
  }
...
export async function fetchMovieCast(movieId) {
  try {
    const res = await fetchData(`/movie/${movieId}/credits`);
    return res;
  } catch (error) {
    throw new Error("Error happened while fetching top rated movies", error);
  }
...
export async function fetchMovieTranslations(movieId) {
  try {
    const res = await fetchData(`/movie/${movieId}/translations`);
    return res;
  } catch (error) {
    throw new Error("Error happened while fetching top rated movies", error);
  }
...
export async function fetchMovieVideos(movieId) {
  try {
    const res = await fetchData(`/movie/${movieId}/videos`);
    return res;
  } catch (error) {
    throw new Error("Error happened while fetching top rated movies", error);
  }
